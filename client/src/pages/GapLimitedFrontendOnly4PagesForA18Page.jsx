...
// Gap (Non-AI): Limited frontend (only 4 pages) for a 18
// Limited frontend (only 4 pages) for a 18-route backend
import React, { useState } from 'react';

export default function GapLimitedFrontendOnly4PagesForA18Page() {
  ...
  const run = async () => {
    ...
    try {
      ...
      const res = await fetch('/api/gap-limited-frontend-only-4-pages-for-a-18/run', {
        method: 'POST',
  ...
  return (
    <div style={{ padding: 24, color: '#e5e7eb' }}>
      <h1 style={{ fontSize: 24, fontWeight: 700, marginBottom: 8 }}>Limited frontend (only 4 pages) for a 18</h1>
      <p style={{ color: '#9ca3af', marginBottom: 16 }}>Limited frontend (only 4 pages) for a 18-route backend</p>

...
// Gap (AI): Filing without '/rejection
// Filing without /rejection-predict (predict likelihood of USPTO office actions)
import React, { useState } from 'react';

export default function GapFilingWithoutRejectionPage() {
  ...
  const run = async () => {
    ...
    try {
      ...
      const res = await fetch('/api/gap-filing-without-rejection/run', {
        method: 'POST',
  ...
  return (
    <div style={{ padding: 24, color: '#e5e7eb' }}>
      <h1 style={{ fontSize: 24, fontWeight: 700, marginBottom: 8 }}>Filing without '/rejection</h1>
      <p style={{ color: '#9ca3af', marginBottom: 16 }}>Filing without /rejection-predict (predict likelihood of USPTO office actions)</p>

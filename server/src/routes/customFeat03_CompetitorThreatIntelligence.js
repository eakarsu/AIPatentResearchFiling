...
// Monitor competitor patent filings in real-time; AI extracts technology trends and flags products you should design around
...
async function callLLM(systemPrompt, userPrompt) {
  ...
  const res = await fetch('https://openrouter.ai/api/v1/chat/completions', {
    ...
    headers: {
      ...
      'X-Title': 'AIPatentResearchFiling / cf-competitor-threat-intelligence',
    },
    body: JSON.stringify({
...
const TABLE_NAME = `custom_feat_cf_competitor_threat_intelligence`;
try {
  await pool.query(`
...
router.post('/run', authenticate, aiRateLimiter, async (req, res) => {
  try {
    ...
    const systemPrompt = `You are an expert assistant helping with the feature: 'Competitor threat intelligence'.
Context: Monitor competitor patent filings in real-time; AI extracts technology trends and flags products you should design around
...
If the user payload is insufficient, populate fields with reasonable defaults and note assumptions in summary.`;
    ...
    console.error('cf-competitor-threat-intelligence/run error:', err);

...
// Background agent that monitors USPTO dockets; auto-responds to office actions with AI-drafted responses; coordinates with inventors for clarifications
...
async function callLLM(systemPrompt, userPrompt) {
  ...
  const res = await fetch('https://openrouter.ai/api/v1/chat/completions', {
    ...
    headers: {
      ...
      'X-Title': 'AIPatentResearchFiling / cf-agentic-patent-prosecution',
    },
    body: JSON.stringify({
...
const TABLE_NAME = `custom_feat_cf_agentic_patent_prosecution`;
try {
  await pool.query(`
...
router.post('/run', authenticate, aiRateLimiter, async (req, res) => {
  try {
    const payload = req.body || {};
    const systemPrompt = `You are an expert assistant helping with the feature: 'Agentic patent prosecution'.
Context: Background agent that monitors USPTO dockets; auto-responds to office actions with AI-drafted responses; coordinates with inventors for clarifications
...
If the user payload is insufficient, populate fields with reasonable defaults and note assumptions in summary.`;
    ...
    console.error('cf-agentic-patent-prosecution/run error:', err);

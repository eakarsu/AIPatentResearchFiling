...
// Gap Feature: Filing without '/rejection
// Filing without /rejection-predict (predict likelihood of USPTO office actions)
...
async function callLLM(systemPrompt, userPrompt) {
  ...
  const res = await fetch('https://openrouter.ai/api/v1/chat/completions', {
    ...
    headers: {
      ...
      'X-Title': 'AIPatentResearchFiling / gap-filing-without-rejection',
    },
    body: JSON.stringify({
...
router.post('/run', auth, aiRateLimiter, async (req, res) => {
  try {
    ...
    const systemPrompt = `You are an expert assistant for the gap feature: 'Filing without \'/rejection'.\nProject: AIPatentResearchFiling.\nContext: Filing without /rejection-predict (predict likelihood of USPTO office actions)\nReturn structured JSON with keys: { summary: string, recommendations: string[], risks: string[], next_steps: string[] }.`;
    ...
    if (_pool && _tableReady) {
      try {
        const r = await _pool.query(
          ...
          ['filing-without-rejection', req.user?.id || null, JSON.stringify(payload), out.content, out.tokensUsed, out.model]
        );
    ...
    res.json({ ok: true, id: savedId, feature: 'filing-without-rejection', result: out.content, tokens_used: out.tokensUsed, model: out.model });
  } catch (e) {

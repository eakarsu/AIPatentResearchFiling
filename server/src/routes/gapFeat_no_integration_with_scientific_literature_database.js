...
// Gap Feature: No integration with scientific literature databases (PubMed, arXiv)
...
async function callLLM(systemPrompt, userPrompt) {
  ...
  const res = await fetch('https://openrouter.ai/api/v1/chat/completions', {
    ...
    headers: {
      ...
      'X-Title': 'AIPatentResearchFiling / gap-no-integration-with-scientific-literature-database',
    },
    body: JSON.stringify({
...
router.post('/run', auth, aiRateLimiter, async (req, res) => {
  try {
    ...
    const systemPrompt = `You are an expert assistant for the gap feature: 'No integration with scientific literature databases (PubMed, arXiv)'.\nProject: AIPatentResearchFiling.\nContext: No integration with scientific literature databases (PubMed, arXiv)\nReturn structured JSON with keys: { summary: string, recommendations: string[], risks: string[], next_steps: string[] }.`;
    ...
    if (_pool && _tableReady) {
      try {
        const r = await _pool.query(
          ...
          ['no-integration-with-scientific-literature-database', req.user?.id || null, JSON.stringify(payload), out.content, out.tokensUsed, out.model]
        );
    ...
    res.json({ ok: true, id: savedId, feature: 'no-integration-with-scientific-literature-database', result: out.content, tokens_used: out.tokensUsed, model: out.model });
  } catch (e) {
...
router.get('/history', auth, aiRateLimiter, async (req, res) => {
  try {
    ...
    const r = await _pool.query(
      ...
      ['no-integration-with-scientific-literature-database']
    );

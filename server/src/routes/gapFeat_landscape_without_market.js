...
// Gap Feature: Landscape without \'/market
// Landscape without /market-threat-score (assess competitive patent risk)
...
async function callLLM(systemPrompt, userPrompt) {
  ...
  const res = await fetchFn('https://openrouter.ai/api/v1/chat/completions', {
    ...
    headers: {
      ...
      'X-Title': 'AIPatentResearchFiling / gap-landscape-without-market',
    },
    body: JSON.stringify({
...
router.post('/run', auth, (req, res, next) => (aiRateLimiter ? aiRateLimiter(req, res, next) : next()), async (req, res) => {
  try {
    ...
    const systemPrompt = `You are an expert assistant for the gap feature: 'Landscape without \'/market'.\nProject: AIPatentResearchFiling.\nContext: Landscape without /market-threat-score (assess competitive patent risk)\nReturn structured JSON with keys: { summary: string, recommendations: string[], risks: string[], next_steps: string[] }.`;
    ...
    if (_pool && _tableReady) {
      try {
        const r = await _pool.query(
          ...
          ['landscape-without-market', req.user?.id || null, JSON.stringify(payload), out.content, out.tokensUsed, out.model]
        );
    ...
    res.json({ ok: true, id: savedId, feature: 'landscape-without-market', result: out.content, tokens_used: out.tokensUsed, model: out.model });
  } catch (e) {

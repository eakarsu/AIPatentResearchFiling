...
// Given competitor product description, AI analyzes whether your patents likely cover it; flag for licensing opportunities
...
async function callLLM(systemPrompt, userPrompt) {
  ...
  const res = await fetch('https://openrouter.ai/api/v1/chat/completions', {
    ...
    headers: {
      ...
      'X-Title': 'AIPatentResearchFiling / cf-claim-infringement-checker',
    },
    body: JSON.stringify({
...
const TABLE_NAME = `custom_feat_cf_claim_infringement_checker`;
try {
  await pool.query(`
...
router.post('/run', authenticate, aiRateLimiter, async (req, res) => {
  try {
    ...
    const systemPrompt = `You are an expert assistant helping with the feature: 'Claim infringement checker'.
Context: Given competitor product description, AI analyzes whether your patents likely cover it; flag for licensing opportunities
...
If the user payload is insufficient, populate fields with reasonable defaults and note assumptions in summary.`;
    ...
    console.error('cf-claim-infringement-checker/run error:', err);

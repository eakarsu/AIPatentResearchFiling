...
// AI evaluates market size, competition, licensing potential by country; recommends which countries to file PCT and when
...
async function callLLM(systemPrompt, userPrompt) {
  ...
  const res = await fetch('https://openrouter.ai/api/v1/chat/completions', {
    ...
    headers: {
      ...
      'X-Title': 'AIPatentResearchFiling / cf-international-filing-optimizer',
    },
    body: JSON.stringify({
...
const TABLE_NAME = `custom_feat_cf_international_filing_optimizer`;
try {
  await pool.query(`
...
router.post('/run', authenticate, aiRateLimiter, async (req, res) => {
  try {
    ...
    const systemPrompt = `You are an expert assistant helping with the feature: 'International filing optimizer'.
Context: AI evaluates market size, competition, licensing potential by country; recommends which countries to file PCT and when
...
If the user payload is insufficient, populate fields with reasonable defaults and note assumptions in summary.`;
    ...
    console.error('cf-international-filing-optimizer/run error:', err);

...
// Compare new patent disclosure against aggregated prior art (patents + literature); provide real-time novelty score and highlight problematic claims
...
async function callLLM(systemPrompt, userPrompt) {
  ...
  const res = await fetch('https://openrouter.ai/api/v1/chat/completions', {
    ...
    headers: {
      ...
      'X-Title': 'AIPatentResearchFiling / cf-novelty-scoring-engine',
    },
    body: JSON.stringify({
...
const TABLE_NAME = `custom_feat_cf_novelty_scoring_engine`;
try {
  await pool.query(`
...
router.post('/run', authenticate, aiRateLimiter, async (req, res) => {
  try {
    ...
    const systemPrompt = `You are an expert assistant helping with the feature: 'Novelty scoring engine'.
Context: Compare new patent disclosure against aggregated prior art (patents + literature); provide real-time novelty score and highlight problematic claims
...
If the user payload is insufficient, populate fields with reasonable defaults and note assumptions in summary.`;
    ...
    console.error('cf-novelty-scoring-engine/run error:', err);
